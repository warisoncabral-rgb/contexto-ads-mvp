...
export default function ExecutorPreflightPanel({ plan, role, approvalResult, result, decision }) {
  ...
  return <section className="panel executor-panel">
    ...
    {manifest && <><div className="executor-facts"><div><span>Manifesto</span><strong>{manifest.manifestHash.slice(0, 12)}…</strong></div><div><span>Operações autorizáveis</span><strong>{protocolReady ? result.protocol.operations.length : manifest.operations.length}</strong></div><div><span>Estado pretendido</span><strong>PAUSED</strong></div><div><span>Executável</span><strong>Não</strong></div></div>
      <div className="operation-list">{manifest.operations.map((operation) => { const observed = result.protocol?.execution?.operations?.find((item) => item.operationKey === operation.operationKey); const reconciled = result.protocol?.reconciledOperations?.find((item) => item.operationKey === operation.operationKey); const normalizedError = observed?.normalizedError ? normalizedErrorLabels[observed.normalizedError] ?? observed.normalizedError : ''; return <div key={operation.operationKey}><span>{operation.order}</span><strong>{operation.action} · {operation.objectType}</strong><small>{reconciled ? `reconciliada · Meta ${reconciled.externalObjectId} · não será recriada` : observed ? `${observed.status}${observed.externalObjectId ? ` · Meta ${observed.externalObjectId}` : ''}${normalizedError ? ` · ${normalizedError}` : ''}${observed.diagnosticCode ? ` · ${observed.diagnosticCode}` : ''}` : 'Não iniciada · execução bloqueada'}</small></div> })}</div>
      {!authorization && canDecide && canPrepareReconciledAttempt && <ControlForm plan={plan} manifest={manifest} approvalId={approvalId} action="prepare_protocol" label="Preparar retomada dos objetos restantes" />}
      {!authorization && canPrepare && !canPrepareReconciledAttempt && <ControlForm plan={plan} manifest={manifest} approvalId={approvalId} action="request_authorization" label="Solicitar autorização curta" />}
