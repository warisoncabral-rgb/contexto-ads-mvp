...
import { validConnectionStart, validOAuthStart, validReadOnlySmokeReport, validTenantAccess } from '../lib/meta-connection-setup.mjs'
...
test('accepts only a complete four-step tenant-scoped read-only smoke report', () => {
  const report = { smokeTestId: '44444444-4444-4444-8444-444444444444', tenantId, connectionId, passed: true, blockers: [], generatedAt: new Date().toISOString(), steps: [
    ['identity', 'passed'], ['asset_discovery', 'passed'], ['capability_validation', 'passed'], ['ad_account_read', 'passed'],
  ].map(([key, status]) => ({ key, status, meaning: 'ok', evidenceRefs: ['evidence:test'] })) }
  assert.equal(validReadOnlySmokeReport(report, tenantId, connectionId), true)
  assert.equal(validReadOnlySmokeReport({ ...report, tenantId: '33333333-3333-4333-8333-333333333333' }, tenantId, connectionId), false)
})
