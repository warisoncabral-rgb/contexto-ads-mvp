...
export const ROSA_VIP_CREATIVE_DRAFT_PRESET = Object.freeze([
  Object.freeze({
    primaryText: 'Quer começar a revender calçados femininos ou ampliar seu mix? A Rosa VIP reúne sapatilhas, mules, rasteirinhas, papetes e salto bloco em modelos, cores e numerações de pronta entrega. Pedido mínimo de R$ 500. Fale com a gente pelo WhatsApp e conheça as opções do atacado.',
    headline: 'Comece seu mix com a Rosa VIP',
    description: 'Calçados femininos no atacado.',
  }),
  Object.freeze({
    primaryText: 'Já vende calçados e procura um fornecedor com qualidade? Conheça o atacado Rosa VIP e encontre diferentes categorias para montar um mix alinhado às suas clientes. Pedido mínimo de R$ 500. Consulte pelo WhatsApp os modelos disponíveis.',
    headline: 'Fornecedor para sua revenda',
    description: 'Consulte modelos, cores e numerações.',
  }),
  Object.freeze({
    primaryText: 'Monte seu pedido de atacado com sapatilhas, mules, rasteirinhas, papetes e salto bloco Rosa VIP. Temos opções de pronta entrega e atendimento direto pelo WhatsApp. Frete grátis e pagamento na entrega são válidos somente nas rotas programadas.',
    headline: 'Conheça o atacado Rosa VIP',
    description: 'Pedido mínimo de R$ 500.',
  }),
])

export const ROSA_VIP_WHATSAPP_MESSAGE =
...
export function validCreativePackage(value, expected) {
  return value && UUID.test(value.creativePackageId)
    ...
    && value.copies.every((copy) => typeof copy.primaryText === 'string'
      ...
      && (copy.whatsappMessage === undefined || typeof copy.whatsappMessage === 'string')
...
export function parseCreativeForm(formData) {
  ...
  const variants = [1, 2, 3].map((index) => {
    ...
    return { index, primaryText: text(`primaryText${suffix}`),
      ...
      whatsappMessage: text(`whatsappMessage${suffix}`),
  ...
  const present = variants.filter((variant) => [variant.primaryText, variant.headline,
    variant.description, variant.whatsappMessage, variant.callToAction, variant.storageRef, variant.sha256,
  ...
  const validVariant = (variant) => variant.primaryText && variant.headline
    ...
    && (variant.callToAction !== 'SEND_WHATSAPP_MESSAGE' || variant.whatsappMessage)
  ...
  return { ok: true, action, tenantId, campaignId, executionPlanId,
    creative: { copies: present.map((variant) => ({ copyId: `copy_variant_${variant.index}`,
      ...
      ...(variant.whatsappMessage ? { whatsappMessage: variant.whatsappMessage } : {}),
