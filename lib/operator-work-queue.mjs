...
function validChange(change, snapshot) {
  return change && SHA.test(change.workItemId) && change.tenantId === snapshot.tenantId
    ...
    && typeof change.blockerCode === 'string' && CHANGE_KINDS.has(change.kind)
    ...
    && typeof change.meaning === 'string' && Array.isArray(change.evidenceRefs)
    && change.evidenceRefs.every((ref) => typeof ref === 'string')
