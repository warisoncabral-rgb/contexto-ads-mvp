...
export function validWorkQueue(payload) {
  ...
  if (Array.isArray(items)) for (const item of items) tenantItemCounts.set(item.tenantId, (tenantItemCounts.get(item.tenantId) ?? 0) + 1)
  return payload && Array.isArray(items) && items.every(validItem)
    ...
    && new Set(snapshotTenantIds).size === snapshotTenantIds.length
    && snapshots.every((snapshot) => tenantItemCounts.has(snapshot.tenantId)
      && snapshot.itemCount === tenantItemCounts.get(snapshot.tenantId))
    && items.every((item) => tenantItemCounts.has(item.tenantId))
