...
function validChangeSemantics(change) {
  if (change.kind === 'entered') return change.previousPriority === null && PRIORITIES.has(change.currentPriority)
  if (change.kind === 'resolved') return PRIORITIES.has(change.previousPriority) && change.currentPriority === null
  if (!PRIORITIES.has(change.previousPriority) || !PRIORITIES.has(change.currentPriority)) return false
  if (change.kind === 'worsened') return PRIORITY_RANK[change.currentPriority] < PRIORITY_RANK[change.previousPriority]
  if (change.kind === 'improved') return PRIORITY_RANK[change.currentPriority] > PRIORITY_RANK[change.previousPriority]
  return change.kind === 'unchanged' && change.currentPriority === change.previousPriority
}
...
function validSnapshot(snapshot) {
  ...
  const validComparison = comparison && typeof comparison.baselineAvailable === 'boolean'
    ...
    && (!comparison.baselineAvailable || (comparison.previousQueueDate !== null && comparison.previousQueueDate < snapshot.queueDate))
...
export function validWorkQueue(payload) {
  ...
  return payload && !Number.isNaN(generatedAtMs)
    && Array.isArray(items) && items.every(validItem)
    && items.every((item) => Date.parse(item.observedAt) <= generatedAtMs)
    && new Set(workItemIds).size === workItemIds.length
    && Array.isArray(snapshots) && snapshots.every(validSnapshot)
    && snapshots.every((snapshot) => Date.parse(snapshot.generatedAt) <= generatedAtMs)
