...
export function validOperationalDecision(decision, tenantId, executionPlanId) {
  return decision
...
export function parseReadinessEvaluation(formData) {
  const tenantId = String(formData.get('tenantId') ?? '').trim()
  const campaignId = String(formData.get('campaignId') ?? '').trim()
  const executionPlanId = String(formData.get('executionPlanId') ?? '').trim()
  return [tenantId, campaignId, executionPlanId].every((value) => UUID_PATTERN.test(value))
...
export async function loadOperationalReadiness({
  ...
  const url = `${base}/v1/operator/tenants/${encodeURIComponent(tenantId)}`
    + `/plans/${encodeURIComponent(executionPlanId)}/readiness`
  ...
  try {
    ...
    if (!validOperationalDecision(decision, tenantId, executionPlanId)) {
