...
function validPlan(plan, tenantId) {
  return plan
    ...
    && (plan.connectionId === undefined || UUID_PATTERN.test(plan.connectionId))
    && (plan.adAccountId === undefined || /^act_\d+$/.test(plan.adAccountId))
    && ((plan.connectionId === undefined) === (plan.adAccountId === undefined))
...
async function safeFetch(url, token, fetchImpl) {
  const configuredTimeout = Number(process.env.CONTEXT_ADS_API_TIMEOUT_MS)
